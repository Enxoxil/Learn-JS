...
function t2(elem) {
  ...
  function r2(element) {
    ...
    Array.from(element).forEach((item) => {
      arr = item.textContent.split(" ");
      arr.forEach((values) => {
        if (parseInt(values)) {
          console.log(typeof parseInt(values), values);
          s = s + +values;
        }
      });
    });
...
// Task 3
// При нажатии кнопки запускается функция t3. Функция t3, с помощью рекурсивной функции r3 должна очистить текст внутри z-3 от
// тегов span, b, i и p. Должен остаться только текст со знаками препинания в переменной out. Функция t3 должна возвращать out.

function t3(elem) {
  let out = elem.textContent;
  r3(elem);
